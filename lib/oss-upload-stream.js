...
function Client(client) {
  ...
  if (!client) {
    throw new Error('Must configure an OSS client before attempting to create an OSS upload stream.');
  }
...
Client.prototype.upload = function (destinationDetails, sessionDetails) {
        ...
        function (err, result) {
          if (err) {
            if(errCnt < partRetryCnt) {
              ...
              abortUpload('Failed to upload a part to OSS: ' + JSON.stringify(err));
          }
          else {
  ...
  var completeUpload = function () {
    ...
    if (multipartUploadID) {
      cachedClient.completeMultipartUpload(
        ...
        function (err, result) {
          if (err)
            abortUpload('Failed to complete the multipart upload on OSS: ' + JSON.stringify(err));
          else {
  ...
  var abortUpload = function (rootError) {
    cachedClient.abortMultipartUpload(
      ...
      function (abortError) {
        if (abortError)
          ws.emit('error', rootError + '\n Additionally failed to abort the multipart upload on OSS: ' + abortError);
        else
          ws.emit('error', rootError);
      }
    );
  ...
  var createMultipartUpload = function () {
    cachedClient.createMultipartUpload(
      ...
      function (err, data) {
        if (err)
          ws.emit('error', 'Failed to create a multipart upload on OSS: ' + JSON.stringify(err));
        else {
