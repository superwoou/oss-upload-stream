...
// Set the OSS client to be used for this upload.
function Client(client, type) {
  if (this instanceof Client === false) {
    return new Client(client, type);
  }

  if (!client) {
    throw new Error('Must configure an '+TYPE+' client before attempting to create an '+TYPE+' upload stream.');
  }
...
// Generate a writeable stream which uploads to a file on OSS.
Client.prototype.upload = function (destinationDetails, sessionDetails) {
  ...
  // Parts which need to be uploaded to OSS.
  var pendingParts = 0;
  ...
  ws.partRetryCnt = function (cnt) {
    if(cnt < 0) cnt = 0;
    partRetryCnt = cnt;
    return ws;
  }

  // Set the maximum amount of data that we will keep in memory before flushing it to OSS as a part
  ...
  // Concurrently upload parts to OSS.
  ...
  var flushPart = function (callback) {
    ...
    var fn = function(errCnt){
      ...
      cachedClient.uploadPart(
        ...
        function (err, result) {
          if (err) {
            if(errCnt < partRetryCnt) {
              fn(errCnt+1);
            }
            else
              abortUpload('Failed to upload a part to '+TYPE+': ' + JSON.stringify(err));
          }
          else {
  ...
  // Turn all the individual parts we uploaded to OSS into a finalized upload.
  var completeUpload = function () {
    ...
    if (multipartUploadID) {
      ...
      opt[(TYPE == "OSS" ? "Complete" : "")+"MultipartUpload"] = {
        Parts: partIds
      };
      cachedClient.completeMultipartUpload(
        ...
        function (err, result) {
          if (err)
            abortUpload('Failed to complete the multipart upload on '+TYPE+': ' + JSON.stringify(err));
          else {
  ...
  var abortUpload = function (rootError) {
    cachedClient.abortMultipartUpload(
      ...
      function (abortError) {
        if (abortError)
          ws.emit('error', rootError + '\n Additionally failed to abort the multipart upload on '+TYPE+': ' + abortError);
        else
          ws.emit('error', rootError);
      }
    );
  ...
  var createMultipartUpload = function () {
    cachedClient.createMultipartUpload(
      ...
      function (err, data) {
        if (err)
          ws.emit('error', 'Failed to create a multipart upload on '+TYPE+': ' + JSON.stringify(err));
        else {
...
Client.upload = function (destinationDetails, sessionDetails) {
  if (!Client.globalClient) {
    throw new Error('Must configure an '+this.TYPE+' client before attempting to create an '+this.TYPE+' upload stream.');
  }
